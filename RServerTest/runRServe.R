...
# "c:\Program Files\R\R-3.2.5\bin\R.exe" -f runRServe.R --gui-none --no-save
...
base.path <- "c:/InnovationJam2016/Source/InnovationJam2016/PriorityInbox/"

## INPUT EMAILS GO HERE
## this is the training data set... 
easyham.path <-paste(base.path,"/data/easy_ham3/", sep="")

## you can call it whatever you want, but only keep 1 file here
onetime.path <-paste(base.path,"data/inbox/", sep="")
onetime.fn   <-paste(base.path,"data/inbox/email.txt", sep="")
...
output.fn5  <-paste(base.path,"output/final_df.csv", sep="")
...
evaluateMsg <- function(emailBody) {

		fileConn<-file(onetime.fn)
		writeLines(trim(emailBody), fileConn)
		close(fileConn)
